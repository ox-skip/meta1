...
const isPublicRoute = (
  ...
  useEffect(() => {
    const userId = user?.id ?? null;
    if (!userId || !lastRouteReady || booting || loading || isPublicRoute) return;
  ...
  const canBypassMaintenance = isAdminRoute || isAuthRoute || isPublicRoute;
  ...
  */
  ...
  if (!user && !isPublicRoute && group !== "(auth)" && group !== "pi") {
